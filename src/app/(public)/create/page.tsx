...
export default function Create() {
  ...
  const registrarContractAddress = useMemo(() => {
    const value = getRegistrarContractAddress(chainId)
  ...
  const performCreation = useCallback(
    async (values: CreateGroupFormValues, feeQuote: PlatformFeeQuote) => {
      ...
      const treasuryAddress = platformTreasuryAddress
      const registrarAddress = registrarContractAddress
      ...
      try {
        ...
        const membershipPriceAmount =
          priceString !== '' ? parseSettlementTokenAmount(priceString) : 0n
        ...
        const feeTx = await sendTransaction(feeQuote.params, {
          ...
          <div className='mx-auto max-w-3xl'>
            ...
            <div className='mb-8 rounded-2xl border border-border/50 bg-card/80 p-6 shadow-lg backdrop-blur-sm'>
              ...
              <p className='mt-4 text-sm text-muted-foreground'>
                Billed once every 30 days. Renew before the window closes to
                keep your community online without interruption.
              </p>
            </div>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className='flex justify-between text-xs text-muted-foreground'>
                          ...
                          <span>
                            {(field.value?.length ?? 0).toString()} / 80
                          </span>
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription className='text-xs text-muted-foreground'>
                            Members are billed this USD amount each month; we
                            settle in MUSD during checkout.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              We&apos;ll charge the monthly platform fee before publishing your
              community.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className='rounded-lg border border-dashed border-border/60 px-3 py-2'>
              ...
              <p className='mt-1 text-sm font-semibold text-foreground'>
                {pendingQuote?.displayAmount ??
                  ...
                  platformFeeSettlementLabel ??
                  'Resolving…'}
              </p>
              <p className='text-xs text-muted-foreground'>
                Paid to treasury{' '}
                {pendingQuote?.treasuryAddress ??
                  cachedPlatformFeeQuote?.treasuryAddress ??
                  platformTreasuryAddress}
                .
              </p>
            </div>
