...
export function GroupMediaFields<FormValues extends GroupMediaFormShape>({
  ...
  useEffect(() => {
    ...
    const nextThumbnailPreview =
      normalizeMediaInput(initialMedia.thumbnailUrl) ||
      nextThumbnailSource ||
      null
    ...
    if (nextGalleryItems.length) {
      ...
      form.setValue(
        galleryPath,
        nextGalleryItems.map(item => item.source) as PathValue<
          FormValues,
          Path<FormValues>
        >,
        { shouldDirty: false }
  ...
  const handleAddGalleryLink = useCallback(() => {
    ...
    form.setValue(
      galleryPath,
      nextItems.map(item => item.source) as PathValue<
        FormValues,
        Path<FormValues>
      >,
      { shouldDirty: true, shouldValidate: true }
                  ...
                  >
                    {thumbnailPreview ? (
                      ...
                      <div className='flex h-full w-full flex-col items-center justify-center gap-2 px-6 text-center text-sm text-muted-foreground'>
                        ...
                        <span>
                          Drag & drop an image to represent your group.
                        </span>
                ...
                <TabsContent value='link'>
                  <FormControl>
                    <Input
                      ...
                      value={
                        isStorageReference(resolvedValue) ? '' : resolvedValue
                      }
                      onBlur={field.onBlur}
                      onChange={event => {
        ...
        render={() => (
          <FormItem>
            ...
            <Tabs
              value={galleryTab}
              onValueChange={value =>
                setGalleryTab(value as 'upload' | 'links')
              }
              className='space-y-3'
            >
              <TabsList className='grid w-full grid-cols-2'>
                ...
                <div className='space-y-3'>
                  <div className='flex flex-col gap-2 sm:flex-row'>
                    <Input
                      ...
                      onChange={event =>
                        setGalleryLinkInput(event.target.value)
                      }
                      onKeyDown={event => {
                        if (event.key === 'Enter') {
            ...
            {galleryItems.length > 0 ? (
              ...
              <div className='mt-4 rounded-lg border border-dashed border-border bg-muted/40 p-6 text-center text-sm text-muted-foreground'>
                No gallery assets yet. Upload files or paste links to showcase
                more visuals.
              </div>
            )}
