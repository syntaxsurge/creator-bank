...
export function GroupAboutSection() {
  ...
  const membershipAddress = useMemo(() => {
    const value = getMembershipContractAddress(chainId)?.trim()
  ...
  const [courseVerification, setCourseVerification] =
    useState<CourseVerificationState>(() =>
      membershipCourseId
        ? { status: 'checking' }
  ...
  useEffect(() => {
    ...
    membershipService
      ...
      .catch(error => {
        ...
        if (!notFound) {
          console.error(
            'Failed to verify membership course for about page',
            error
          )
        }
        setCourseVerification(
  ...
  useEffect(() => {
    ...
    const walletAddress = (address ?? currentUser?.walletAddress) as
      | Address
      | undefined
    ...
    membershipService
      ...
      .catch(error => {
        console.error(
          'Failed to resolve membership expiry for about page',
          error
        )
      })
  ...
  const explorerBaseUrl = explorerUrl
  const transactionHash = subscription.lastPaymentTxHash?.trim()
  const membershipCourseIdLabel = membershipCourseId
    ? membershipCourseId.toString()
    : 'Not assigned'
  const explorerName = 'Mezo Explorer'
  ...
  }, [courseVerification, explorerName, paymentExplorerUrl])
