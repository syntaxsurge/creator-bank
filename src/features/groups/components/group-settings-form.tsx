...
export function GroupSettingsForm({ group }: GroupSettingsFormProps) {
  ...
  const resetSubscriptionIdMutation = useMutation(
    api.groups.resetSubscriptionId
  )
  ...
  const registrarAddress = useMemo(() => {
  ...
  }, [address, membershipAddress, publicClient, walletClient])
  ...
  }, [address, publicClient, walletClient, registrarAddress])
  ...
  const handleRegisterCourse = useCallback(async () => {
    ...
    if (!walletClient || !registrarService) {
    ...
    try {
      ...
      const priceValue =
        typeof group.price === 'number' && Number.isFinite(group.price)
          ? group.price
          : 0
      const membershipPriceAmount = parseSettlementTokenAmount(
        priceValue.toString()
      )
          ...
          <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
            <div className='space-y-1'>
              ...
              <p className='text-xs text-muted-foreground'>
                {subscriptionCardDescription} Pay {platformFeeLabel} to extend
                access for 30 days.
              </p>
            </div>
        ...
        <div className='space-y-3 rounded-xl border border-border p-4'>
          <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
            <div>
              ...
              <p className='text-xs text-muted-foreground'>
                Register this group&apos;s course on-chain so you can enable
                paid memberships or marketplace listings later.
              </p>
            </div>
