...
import { getMezoPublicClient } from '@/lib/onchain/mezo-chain'
import {
  MarketplaceService,
...
export function useMarketplaceCore() {
  ...
  const { address, walletClient } = useWalletAccount()
  ...
  const publicClient = useMemo(() => getMezoPublicClient(chainId), [chainId])

  const marketplaceAddress = useMemo(() => {
    const value = getMarketplaceContractAddress(chainId)
    return value ? (value as Address) : undefined
  }, [chainId])
  const membershipAddress = useMemo(() => {
    const value = getMembershipContractAddress(chainId)
    return value ? (value as Address) : undefined
  }, [chainId])
  ...
  const marketplaceService = useMemo(() => {
    ...
    return new MarketplaceService({
      ...
      account: address ? (address as Address) : null,
      address: marketplaceAddress
    })
  }, [address, publicClient, walletClient, marketplaceAddress])
  ...
  }, [address, publicClient, walletClient, membershipAddress])

  const musdService = useMemo(() => {
    if (!musdAddress) return null
  ...
  const ensureAllowance = useCallback(
    async (amount: bigint) => {
      if (!musdService || !address || !marketplaceAddress) {
        throw new Error(
          'Settlement token configuration missing. Contact support.'
        )
      }
      if (amount === 0n) return
      const currentAllowance = await musdService.allowance(
        address as Address,
        marketplaceAddress
      )
      if (currentAllowance < amount) {
        const approvalTx = await musdService.approve(marketplaceAddress, amount)
        await approvalTx.wait()
      }
    },
    [address, marketplaceAddress, musdService]
  )
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    enabled: Boolean(marketplaceService && membershipService),
    queryFn: async (): Promise<MarketplaceCourse[]> => {
      if (!marketplaceService || !membershipService) return []
        ...
        dynamicCatalog.map(async catalogItem => {
          ...
          try {
            const [courseConfig, listings] = await Promise.all([
              membershipService.getCourse(courseId),
              marketplaceService.getActiveListings(courseId)
            ])

            let userState: MarketplaceCourse['user']
            if (address) {
              const [balance, transferInfo, passState] = await Promise.all([
                membershipService.balanceOf(address as Address, courseId),
                membershipService.canTransfer(courseId, address as Address),
                membershipService.getPassState(courseId, address as Address)
              ])
            ...
            const floor = listings.reduce<bigint | null>((lowest, listing) => {
              if (!lowest || listing.price < lowest) return listing.price
              return lowest
            }, null)
            ...
            return {
              ...
              stats: {
                ...
                primaryPrice: courseConfig.price,
  ...
  const handlePrimaryPurchase = async (course: MarketplaceCourse) => {
    if (!walletClient || !marketplaceService) {
      toast.error('Connect your wallet to purchase memberships')
      return
    }
    try {
      await ensureAllowance(course.stats.primaryPrice)
  ...
  const handleBuyFloor = async (course: MarketplaceCourse) => {
    ...
    try {
      await ensureAllowance(floorListing.price)
      const tx = await marketplaceService.buyListing(
        course.catalog.courseId,
        floorListing.seller,
        floorListing.price
      )
      ...
      console.error(error)
      toast.error(
        error?.shortMessage ?? error?.message ?? 'Unable to buy listing'
      )
    }
  }
  ...
  const handleRenew = async (course: MarketplaceCourse) => {
    ...
    try {
      await ensureAllowance(course.stats.primaryPrice)
      const tx = await marketplaceService.renew(
        course.catalog.courseId,
        course.stats.primaryPrice
      )
      await tx.wait()
      ...
      console.error(error)
      toast.error(
        error?.shortMessage ?? error?.message ?? 'Unable to renew membership'
      )
    }
  }
  ...
  const handleCreateListing = async (payload: {
    ...
    try {
      const priceAmount = parseSettlementTokenAmount(payload.price)
      const approvalGranted = await membershipService.isApprovedForAll(
        address as Address,
        marketplaceAddress as Address
      )

      if (!approvalGranted) {
        const approvalTx = await membershipService.setApprovalForAll(
          marketplaceAddress as Address,
          true
        )
        await approvalTx.wait()
...
export function MarketplaceShell() {
  ...
  const { quote: platformFeeQuote, label: platformFeeLabel } =
    usePlatformFeeQuote()
  const { chainId } = useChainPreference()
  const treasuryAddress = useMemo(
    () => getPlatformTreasuryAddress(chainId) || '',
    [chainId]
  )
  const marketplaceAddressDisplay = useMemo(
    () => getMarketplaceContractAddress(chainId) || '',
    [chainId]
  )
            ...
            <div className='space-y-4 rounded-lg bg-muted/30 p-4'>
              <div>
                ...
                <p className='mt-1 text-xs text-muted-foreground'>
                  Settlement asset:{' '}
                  {platformFeeQuote?.symbol ?? SETTLEMENT_TOKEN_SYMBOL}
                </p>
              </div>
            </div>
            ...
            <div className='space-y-3 text-xs'>
              <div>
                ...
                <p className='mt-1 break-all font-mono text-[0.65rem] text-foreground'>
                  {platformFeeQuote?.treasuryAddress ?? treasuryAddress}
                </p>
              </div>
              <div>
    ...
    <div className='relative overflow-hidden rounded-2xl border border-border/30 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-10 py-16 text-white shadow-2xl md:px-14'>
      ...
      <div className='relative space-y-6'>
        ...
        <p className='max-w-2xl text-lg leading-relaxed text-slate-300'>
          Buy new passes, discover secondary listings, or renew existing
          memberships. All transactions settle on Mezo with cooldown-aware
          transfers for safer trading.
        </p>
    ...
    <div className='group flex h-full flex-col overflow-hidden rounded-xl border border-border/50 bg-card/80 shadow-lg backdrop-blur-sm transition-all hover:shadow-xl'>
      ...
      <div className='flex flex-1 flex-col gap-4 p-6'>
        <div className='flex items-start justify-between gap-3'>
          ...
          <div className='flex flex-col items-end gap-1 rounded-lg bg-primary/10 px-3 py-2'>
            ...
            <p className='text-lg font-bold text-foreground'>
              {formatSettlementToken(course.stats.primaryPrice)}
            </p>
            {course.floorPrice && (
              <p className='text-xs font-medium text-accent'>
                Floor:{' '}
                {course.floorPrice
                  ? formatSettlementToken(course.floorPrice)
                  : 'No listings yet'}
              </p>
            )}
          </div>
        ...
        <dl className='grid gap-3 rounded-lg bg-muted/40 p-4 text-xs sm:grid-cols-2'>
          ...
          />
          <StatItem label='Next Expiry' value={nextListingExpiryLabel} />
        </dl>
              ...
              {course.listings.slice(0, 2).map(listing => (
                ...
                >
                  ...
                  <span className='font-semibold text-foreground'>
                    {formatSettlementToken(listing.price)}
                  </span>
                </div>
              ))}
        ...
        {userHasPass ? (
          <div className='rounded-lg border border-emerald-500/40 bg-emerald-500/10 p-3 text-xs text-emerald-100'>
            ...
            <div className='mt-2 grid gap-2 sm:grid-cols-2'>
              <div>
                <p className='text-[0.625rem] uppercase tracking-wider text-emerald-200/80'>
                  Expires
                </p>
        ...
        <div className='space-y-4'>
          ...
          <div className='space-y-2'>
            <Label htmlFor='listing-price'>
              Listing price ({SETTLEMENT_TOKEN_SYMBOL})
            </Label>
            <Input
              id='listing-price'
              type='number'
