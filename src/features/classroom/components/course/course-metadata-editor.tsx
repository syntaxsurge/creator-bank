...
export function CourseMetadataEditor({
  ...
  const { mutate: updateThumbnail, pending: thumbnailPending } = useApiMutation(
    api.courses.updateThumbnail
  )
  ...
  const handleThumbnailFiles = async (files: File[]) => {
    ...
    try {
      ...
      toast.error(
        'Unable to upload that image. Try again with a different file.'
      )
    } finally {
                    ...
                    >
                      {thumbnailPreviewLoading ? (
                        ...
                        <div className='flex h-full w-full flex-col items-center justify-center gap-2 px-6 text-center text-sm text-muted-foreground'>
                          <ImageIcon className='h-6 w-6' />
                          <span>
                            Drag & drop an image to represent your course.
                          </span>
                          <span className='text-xs text-muted-foreground'>
                            PNG, JPG, or GIF up to 5MB.
                          </span>
                  ...
                  <TabsContent value='link'>
                    <div className='space-y-2'>
                      <Input
                        ...
                        value={thumbnailLinkInput}
                        onChange={event =>
                          setThumbnailLinkInput(event.target.value)
                        }
                        onBlur={() => {
                          void handleThumbnailLinkCommit()
                        }}
                      ...
                      />
                      <p className='text-xs text-muted-foreground'>
                        Paste a direct image URL. JPG, PNG, or GIF files work
                        best.
                      </p>
                    </div>
                  </TabsContent>
              ...
              >
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      This action removes all modules, lessons, and related
                      posts for this course. It cannot be undone.
                    </DialogDescription>
                  </DialogHeader>
