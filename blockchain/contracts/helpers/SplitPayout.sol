...
/**
 * @title SplitPayout
 * @notice Pull-based MUSD revenue splitter used by CreatorBank courses.
 */
contract SplitPayout is ReentrancyGuard {
    ...
    function recordPayment(address payer, uint256 amount) external onlyAuthorized {
        ...
        uint256 currentBalance = payoutToken.balanceOf(address(this));
        if (currentBalance < totalReceived + amount) revert InsufficientPayoutBalance();
