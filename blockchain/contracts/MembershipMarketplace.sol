 ...
 * @notice Facilitates primary sales, renewals, and secondary listings for CreatorBank memberships using MUSD.
...
contract MembershipMarketplace is AccessControl, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        if (address(membershipContract) == address(0) || address(paymentToken_) == address(0)) {
        ...
        whenNotPaused
    {
        ...
        uint256 price = courseInfo.price;
        if (price == 0) revert InvalidPrice();
        if (price > maxPrice) revert InvalidPrice();

        _collectPayment(price);

        uint256 feeAmount = _applyFee(price);
        uint256 payout = price - feeAmount;

        if (payout > 0) {
            paymentToken.safeTransfer(courseInfo.splitter, payout);
            ISplitPayout(courseInfo.splitter).recordPayment(msg.sender, payout);
        }

        membership.marketplaceMint(msg.sender, courseId, price);

        emit PrimaryPurchase(courseId, msg.sender, price, feeAmount);
    }

    function createListing(uint256 courseId, uint256 price, uint64 durationSeconds)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        emit ListingCreated(courseId, msg.sender, price, listing.listedAt, expiresAtTs, expiresAt);
    }
        ...
        whenNotPaused
    {
        ...
        emit ListingPurchased(courseId, seller, msg.sender, price, feeAmount);
    }
    ...
    function renew(uint256 courseId, uint256 maxPrice) external nonReentrant whenNotPaused {
        ...
        emit PassRenewedOnMarketplace(courseId, msg.sender, price, feeAmount, state.expiresAt);
    }
    ...
    function _applyFee(uint256 amount) internal returns (uint256 feeAmount) {
        if (platformFeeBps == 0 || amount == 0) {
            return 0;
        }
        feeAmount = (amount * platformFeeBps) / FEE_DENOMINATOR;
        if (feeAmount > 0) {
