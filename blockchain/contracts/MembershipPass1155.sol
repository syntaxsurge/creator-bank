 ...
 * @notice ERC-1155 contract that mints access passes for CreatorBank courses in exchange for MUSD.
...
contract MembershipPass1155 is ERC1155Supply, AccessControl, Pausable, ReentrancyGuard {
    ...
    function createCourse(
        ...
        emit CourseCreated(courseId, price, splitter, creator, duration, transferCooldown);
    }

    function setPrice(uint256 courseId, uint256 newPrice) external whenNotPaused {
        ...
        uint256 oldPrice = courseInfo.price;
        courseInfo.price = newPrice;
        ...
        onlyRole(MARKETPLACE_ROLE)
    {
        ...
        if (courseInfo.price != 0 && amountPaid < courseInfo.price) {
            revert AmountMismatch(courseInfo.price, amountPaid);
        }
