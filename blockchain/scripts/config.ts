/**
 * CreatorBank deployment configuration.
 */
...
export const existingRegistrarAddress = env.REGISTRAR_CONTRACT_ADDRESS ? normalise(env.REGISTRAR_CONTRACT_ADDRESS) : "";

export const membershipDurationSeconds = parseUintEnv("MEMBERSHIP_DURATION_SECONDS", 60 * 60 * 24 * 30); // 30 days
export const membershipTransferCooldownSeconds = parseUintEnv("MEMBERSHIP_TRANSFER_COOLDOWN_SECONDS", 60 * 60 * 24); // 1 day

export const musdTokenAddress = env.MUSD_CONTRACT_ADDRESS ? normalise(env.MUSD_CONTRACT_ADDRESS) : "";
if (!musdTokenAddress) {
  throw new Error("MUSD_CONTRACT_ADDRESS must be provided for MUSD settlement");
}
...
export const marketplaceMaxListingDuration = parseUintEnv("MARKETPLACE_MAX_LISTING_DURATION_SECONDS", 60 * 60 * 24 * 7); // 7 days
